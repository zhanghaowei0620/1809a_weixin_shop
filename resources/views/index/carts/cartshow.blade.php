...
<a href="address?order_id={{$amount->order_id}}">
<div class="userinfo">
@if($addressnum==0)
        ...
        @else
            <div class="express-bottom">
                <ul class="clearfix">
                    ...
                    <li class="info">
                        <div class="clearfix">
                            <span class="user fl">{{$addressDefault['add_name']}}</span>
                            <span class="tel fr">{{$addressDefault['add_tel']}}</span>
                        </div>
                        <p class="noaddr">{{$addressDefault['add_province'].$addressDefault['add_details']}}</p>
                    </li>
                    <li><em></em></li>
                </ul>
            </div>
                ...
                <li>
                    <a href="">
                        ...
                        <dl>
                            ...
                            <dd>￥<em class="price">{{$v->goods_selfprice}}</em>   <em>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;数量： </em><em class="zxczc">{{$v->buy_number}}</em></dd>
                        </dl>
                    </a>
                </li>
                @endforeach
        ...
        <div class="other_pay marginB">
            ...
            <div class="net-pay">
                <a href="javascript:;" class="checked" id="jdPay" >
                    <span class="zfb"></span>
                    <b class="z-set" id="aa"></b>
                </a>
                <a href="javascript:;" id="jdPay">
                    <img src="https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=3336519328,2903738409&fm=26&gp=0.jpg" style="height: 30px;width: 30px;">
                    <font style="width: 70px;height: 70px;">微信支付</font>
                    <b class="z-set" id="bb"></b>
                </a>
            </div>
            <div class="paylip">我们提倡理性消费</div>
        </div>
        <div class="g-Total-bt">
            <dd id="cc"><a id="sub" href="payindex" class="orangeBtn fr w_account">立即支付</a></dd>
        </div>
...
<script>
    ...
    $('#bb').click(function(){
        $('#cc').html('<a id="sub" href="wpay" class="orangeBtn fr w_account">立即支付</a>');
    });
    $('#aa').click(function(){
        $('#cc').html('<a id="sub" href="payindex" class="orangeBtn fr w_account">立即支付</a>');
    });
	...
	$(document).ready(function(){
		...
		for(var i = 0;i<$('.g-pay-lst li').length;i++){
			total +=parseInt($('.g-pay-lst li').eq(i).find('dd em.price').text())*parseInt($('.zxczc').text());
		}
		$('.gray9 .orange').html('<i>￥</i>'+total.toFixed(2));
		$('.wzf .orange').html('<span class="colorbbb">需要支付&nbsp;</span><b>￥</b>'+total.toFixed(2));
        $('.money').html('<i>￥</i>'+total.toFixed(2));
		// 判断选择余额支付还是潮购值支付
