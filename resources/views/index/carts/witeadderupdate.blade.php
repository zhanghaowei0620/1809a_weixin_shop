...
<form class="layui-form" action="">
  <div class="addrcon">
    ...
    <li><em>收货人</em><input type="text" name="add_name"  value="{{$data->add_name}}" placeholder="请填写真实姓名"></li>
      <li><em>手机号码</em><input type="number" name="add_tel" value="{{$data->add_tel}}" placeholder="请输入手机号"></li>
      <li><em>所在区域</em><input  type="text" name="add_province" value="{{$data->add_province}}"  placeholder="请选择所在区域"></li>
      <li class="addr-detail"><em>详细地址</em><input type="text" value="{{$data->add_details}}" name="add_details" placeholder="20个字以内" class="addr"></li>
    </ul>
...
<script>
  $('.m-index-icon').click(function(){
    ...
    $.post(
      'witeaddrupdate',
      {add_name:add_name,add_tel:add_tel,add_province:add_province,add_details:add_details,is_default:is_default,add_id:add_id},
      function(res){
        console.log(res)
        if(res == 'ok'){
